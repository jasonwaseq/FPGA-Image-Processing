...
def test_each(test_name, simulator):
    ...
    runner(simulator, timescale, tbpath, parameters, testname=test_name, pymodule="test_conv1d")
...
def test_all(simulator):
    ...
    parameters = dict(locals())
    del parameters['simulator']
    runner(simulator, timescale, tbpath, parameters, pymodule="test_conv1d")
...
def test_lint(simulator):
    ...
    parameters = dict(locals())
    del parameters['simulator']
    lint(simulator, timescale, tbpath, parameters, pymodule="test_conv1d")
...
def test_style(simulator):
    ...
    parameters = dict(locals())
    del parameters['simulator']
    lint(simulator, timescale, tbpath, parameters, compile_args=["--lint-only", "-Wwarn-style", "-Wno-lint"], pymodule="test_conv1d")

class Conv1dModel():
    def __init__(self, dut):

        self._f = [0, -2, -4, -8, -8, -16, -16, -16, 128, -16, -16, -16, -8, -8, -4, -2, 0]
        self._dut = dut
        ...
        self._delay = len(self._f)
        self._buf = [-1] * self._delay
        self._deqs = 0
        self._enqs = 0
    ...
    def produce(self):
        ...
        #print(self._buf, self._deqs)
        if(self._deqs >= self._delay):
            elw = [(b * f) for b, f in zip(self._buf, self._f)]
            elwd = [int(x/128) for x in elw]
            expected = sum(elwd)
...
async def single_test(dut):
    ...
    m = ModelRunner(dut, Conv1dModel(dut))
...
async def out_fuzz_test(dut):
    """Transmit 256 (counting) data elements at 50% line rate (Output/Consumer is fuzzed)"""
...
async def in_fuzz_test(dut):
    """Transmit 256 (counting) data elements at 50% line rate (Input/Producer is fuzzed)"""
...
async def inout_fuzz_test(dut):
    """Transmit 256 (counting) random data elements at ~25% line rate (Both are fuzzed)"""
...
async def full_bw_test(dut):
    """Transmit 256 (counting) random data elements at 100% line rate"""
